...
print("max_temperature:", df_health["temperature"].max())
...
def update_kpis(
    ...
    cpu_index, cpu_list, mem_index, mem_list, disk_index, disk_list, temp_index, temp_list
):
    ...
    # CPU
    if triggered_id == "cpu-prev" and cpu_index > 0:
        cpu_index -= 1
    elif triggered_id == "cpu-next" and cpu_index < len(cpu_list) - 1:
        cpu_index += 1

    # Memory
    if triggered_id == "memory-prev" and mem_index > 0:
        mem_index -= 1
    elif triggered_id == "memory-next" and mem_index < len(mem_list) - 1:
        mem_index += 1

    # Disk
    if triggered_id == "disk-prev" and disk_index > 0:
        disk_index -= 1
    elif triggered_id == "disk-next" and disk_index < len(disk_list) - 1:
        disk_index += 1

    # Temperature
    if triggered_id == "temperature-prev" and temp_index > 0:
        temp_index -= 1
    elif triggered_id == "temperature-next" and temp_index < len(temp_list) - 1:
        temp_index += 1

    return (
        cpu_list[cpu_index] if cpu_list else "No data",
        mem_list[mem_index] if mem_list else "No data",
        disk_list[disk_index] if disk_list else "No data",
        temp_list[temp_index] if temp_list else "No data",
        cpu_index,
        mem_index,
        disk_index,
        temp_index
...
def update_visuals_for_Health(start_date, end_date):
    ...
    filtered_health = filtered_health[(filtered_health["timestamp"] >= start_dt) & (filtered_health["timestamp"] <= end_dt)]
    print("max_temperature2:", filtered_health['temperature'].max())
 ### cpu_usage
    max_cpu = filtered_health["cpu_usage"].max()
    # Filter rows with peak CPU usage
    peak_cpu_df = filtered_health[filtered_health["cpu_usage"] == max_cpu].copy()
    # Convert timestamp to date string
    peak_cpu_df["date"] = peak_cpu_df["timestamp"].dt.strftime("%Y-%m-%d")
    # Weekday (if not already present)
    if "weekday" not in peak_cpu_df.columns:
        peak_cpu_df["weekday"] = peak_cpu_df["timestamp"].dt.strftime("%A")
    # Hour from timestamp (if not already present)
    if "hour" not in peak_cpu_df.columns:
        peak_cpu_df["hour"] = peak_cpu_df["timestamp"].dt.hour.astype(int)
    print("a",peak_cpu_df)
    # print("b",peak_cpu_df["hour"].isna().sum())  # Any NaNs?
    # print("c",peak_cpu_df["hour"].apply(lambda x: isinstance(x, int)).value_counts())
    if not peak_cpu_df.empty:
        peak_cpu_df[["start_hour", "end_hour"]] = peak_cpu_df["hour"].apply(
            lambda h: pd.Series(get_hour_range(h))
        )
        peak_cpu_df["summary"] = peak_cpu_df.apply(
            lambda row: f"{row['cpu_usage']}%, {row['date']}, {row['weekday']}, {row['start_hour']}-{row['end_hour']}",
            axis=1
        )
        peak_cpu_df = peak_cpu_df.sort_values(by=["date", "hour"], ascending=[True, True])
        last_peak_cpu_10_summaries = peak_cpu_df["summary"].tail(50).tolist()
    else:
        last_peak_cpu_10_summaries = None

####memory_usage
    max_memory_usage = filtered_health["memory_usage"].max()
    peak_memory_df = filtered_health[filtered_health["memory_usage"] == max_memory_usage].copy()
    # Convert timestamp to date string
    peak_memory_df["date"] = peak_memory_df["timestamp"].dt.strftime("%Y-%m-%d")
    # Weekday (if not already present)
    if "weekday" not in peak_memory_df.columns:
        peak_memory_df["weekday"] = peak_memory_df["timestamp"].dt.strftime("%A")
    # Hour from timestamp (if not already present)
    if "hour" not in peak_memory_df.columns:
        peak_memory_df["hour"] = peak_memory_df["timestamp"].dt.hour.astype(int)
    if not peak_memory_df.empty:
        peak_memory_df[["start_hour", "end_hour"]] = peak_memory_df["hour"].apply(
            lambda h: pd.Series(get_hour_range(h))
        )
        peak_memory_df["summary"] = peak_memory_df.apply(
            lambda row: f"{row['memory_usage']} MB, {row['date']}, {row['weekday']}, {row['start_hour']}-{row['end_hour']}",
            axis=1
        )
        peak_memory_df = peak_memory_df.sort_values(by=["date", "hour"], ascending=[True, True])
        last_peak_memory_10_summaries = peak_memory_df["summary"].tail(50).tolist()
    else:
        last_peak_memory_10_summaries = None

    print("last_peak_memory:", last_peak_memory_10_summaries)



####disk_usage
    max_disk_usage = filtered_health["disk_usage"].max()
    peak_disk_df = filtered_health[filtered_health["disk_usage"] == max_disk_usage].copy()
    # Convert timestamp to date string
    peak_disk_df["date"] = peak_disk_df["timestamp"].dt.strftime("%Y-%m-%d")
    # Weekday (if not already present)
    if "weekday" not in peak_disk_df.columns:
        peak_disk_df["weekday"] = peak_disk_df["timestamp"].dt.strftime("%A")
    # Hour from timestamp (if not already present)
    if "hour" not in peak_disk_df.columns:
        peak_disk_df["hour"] = peak_disk_df["timestamp"].dt.hour.astype(int)
    if not peak_disk_df.empty:
        peak_disk_df[["start_hour", "end_hour"]] = peak_disk_df["hour"].apply(
            lambda h: pd.Series(get_hour_range(h))
        )
        peak_disk_df["summary"] = peak_disk_df.apply(
            lambda row: f"{row['disk_usage']}%, {row['date']}, {row['weekday']}, {row['start_hour']}-{row['end_hour']}",
            axis=1
        )
        peak_disk_df = peak_disk_df.sort_values(by=["date", "hour"], ascending=[True, True])
        last_peak_disk_10_summaries = peak_disk_df["summary"].tail(50).tolist()
    else:
...
####temperature
    ...
    peak_temperature_df = filtered_health[filtered_health["temperature"] == max_temperature].copy()
    # Convert timestamp to date string
    peak_temperature_df["date"] = peak_temperature_df["timestamp"].dt.strftime("%Y-%m-%d")
    # Weekday (if not already present)
    if "weekday" not in peak_temperature_df.columns:
        peak_temperature_df["weekday"] = peak_temperature_df["timestamp"].dt.strftime("%A")
    # Hour from timestamp (if not already present)
    if "hour" not in peak_temperature_df.columns:
        peak_temperature_df["hour"] = peak_temperature_df["timestamp"].dt.hour.astype(int)
    if not peak_temperature_df.empty:
        peak_temperature_df[["start_hour", "end_hour"]] = peak_temperature_df["hour"].apply(
            lambda h: pd.Series(get_hour_range(h))
        )
        peak_temperature_df["summary"] = peak_temperature_df.apply(
            lambda row: f"{row['temperature']} C, {row['date']}, {row['weekday']}, {row['start_hour']}-{row['end_hour']}",
            axis=1
        )
        peak_temperature_df = peak_temperature_df.sort_values(by=["date", "hour"], ascending=[True, True])
        last_peak_temperature_10_summaries = peak_temperature_df["summary"].tail(50).tolist()
    else:
        last_peak_temperature_10_summaries = None

    # print("last_peak_temperature_10_summaries: ", peak_temperature_df.tail(50))

    kpi_blocks = [
    dcc.Store(id="cpu-kpi-index", data=len(last_peak_cpu_10_summaries) - 1 if last_peak_cpu_10_summaries else 0),
    dcc.Store(id="cpu-kpi-list", data=last_peak_cpu_10_summaries),  # REQUIRED for arrow navigation
    dcc.Store(id="memory-kpi-index", data=len(last_peak_memory_10_summaries) - 1 if last_peak_memory_10_summaries else 0),
    dcc.Store(id="memory-kpi-list", data=last_peak_memory_10_summaries),
    dcc.Store(id="disk-kpi-index", data=len(last_peak_disk_10_summaries) - 1 if last_peak_disk_10_summaries else 0),
    dcc.Store(id="disk-kpi-list", data=last_peak_disk_10_summaries),
    dcc.Store(id="temperature-kpi-index", data=len(last_peak_temperature_10_summaries) - 1 if last_peak_temperature_10_summaries else 0),
