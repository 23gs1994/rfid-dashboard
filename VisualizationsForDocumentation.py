...
def update_visuals(device_id, start_date, end_date):
    ...
    tag_fig = go.Figure([
        go.Bar(x=tag_df["device_id_id"], y=tag_df["total_tag_reads"], marker_color="red", width=[0.4] * len(tag_df))
    ])
    ...
    session_fig = go.Figure([
        go.Bar(x=session_df["device_id_id"], y=session_df["session_count"], marker_color="green", width=[0.4] * len(session_df))
    ])
